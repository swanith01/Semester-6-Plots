"""
stitch_lightcones_3D.py
=======================
400 Mpc / 64^3 lightcone stitching for 5 seeds — 3D version.
Fields: xHI, density, vz, halos, LAEs, LBGs.

Output lightcones are 3D: shape (NGRID, NGRID, N_LC_PIX) = (64, 64, 512).
This preserves the full transverse plane at every LoS pixel, enabling
proper 2D projected maps and cross-power spectrum computation.

Previous 2D version (X_SLICE only) preserved in:
    26Jun2026_400Mpc_halo_LAE_LBG/  (2D, working)
...
Usage:
    python stitch_lightcones_3D.py              # all seeds
    python stitch_lightcones_3D.py --seed 3     # single seed
    python stitch_lightcones_3D.py --seed 1 3 5 # multiple seeds
"""
...
# X_SLICE removed — 3D version saves full (NGRID, NGRID) slab at every LoS pixel
...
# ── Output ────────────────────────────────────────────────────────────────────
OUT_ROOT    = "/user1/swanith/kSZ2_halo_project/27Jun2026_400Mpc_halo_LAE_LBG_3D"
...
def stitch_continuous(seed, field_name, snap_z, z_arr, logger):
    """
    Stitch a continuous field (xHI, density, vz) into a 3D lightcone.
    Interpolates across snapshots for each LoS pixel.
    Returns (NGRID, NGRID, N_LC_PIX) array — full transverse plane preserved.
    """
    lc = np.zeros((NGRID, NGRID, N_LC_PIX), dtype=np.float32)
    ...
    for n, z in enumerate(z_arr):
        ...
        # Interpolate in z for each transverse cell — store full (NGRID, NGRID) slab
        interp = interp1d(loaded_z, slabs, axis=-1,
                          bounds_error=False, fill_value='extrapolate')
        lc[:, :, n] = interp(z)           # full transverse plane
...
def stitch_discrete(seed, field_name, snap_z, z_arr, logger):
    """
    Stitch a discrete field (halos, LAEs, LBGs) into a 3D lightcone.
    ...
    for n, z in enumerate(z_arr):
        ...
        lc[:, :, n] = slab               # full transverse plane
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*60}")
    print(f"400 Mpc Lightcone Stitching (3D) — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"Output shape per field: ({NGRID}, {NGRID}, {N_LC_PIX})")
